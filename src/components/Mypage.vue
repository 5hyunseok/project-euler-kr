  ...
  <div id="progress_page">
    ...
    <div id="problems_solved_section">
      ...
      <table class="grid problems_solved_table">
        ...
        <td :class="{ problem_unsolved: !problem.solved, problem_solved: problem.solved }" 
        v-bind:style="{ backgroundColor: problem.solved ? 'rgba(255, 136, 0,0.335)' : ''}"
        v-for="problem in totalProblemList[i]" :key="problem.id">
  ...
  created() {
    this.$http.get(`${baseURI}/problems/length`)
      .then((result) => {
        ...
        let len = Math.trunc(this.totalNumber / 20);
        ...
        this.$http.get(`${baseURI}/users/my`, {
          ...
          .then((result) => {
            this.totalPostNumber = result.data.threadCount;
            this.totalStarNumber = result.data.threadStarCount;
